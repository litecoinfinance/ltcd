...
var helpDescsEnUS = map[string]string{
	...
	"getblock-verbosetx":   "Specifies that each transaction is returned as a JSON object and only applies if the verbose flag is true (ltfnd extension)",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (ltfnd does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown ltfnd.",
	"stop--result0":  "The string 'ltfnd stopping.'",
	...
	// NotifySpentCmd help.
	"notifyspent--synopsis": "Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this ltfnd instance) and when such a transaction first appears in a newly-attached block.",
