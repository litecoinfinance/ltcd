...
var (
	...
	ltfnctlHomeDir         = ltfnutil.AppDataDir("ltfnctl", false)
	ltfnwalletHomeDir      = ltfnutil.AppDataDir("ltfnwallet", false)
	defaultConfigFile     = filepath.Join(ltfnctlHomeDir, "ltfnctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(ltfndHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(ltfnwalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for ltfnctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(ltfnctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		...
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(ltfnwalletHomeDir, "ltfnwallet.conf")
		} else {
			serverConfigPath = filepath.Join(ltfndHomeDir, "ltfnd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either ltfnd or
// ltfnwallet), and extract the RPC user and password from it.
