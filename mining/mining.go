...
const (
	...
	MinHighPriority = ltfnutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr ltfnutil.Address) (*ltfnutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *ltfnutil.Tx, height int32) error {
...
func logSkippedDeps(tx *ltfnutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress ltfnutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*ltfnutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := ltfnutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := ltfnutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A ltfnutil.Block should use saved in the state to avoid
