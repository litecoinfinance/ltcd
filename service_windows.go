...
const (
	// svcName is the name of ltfnd service.
	svcName = "ltfndsvc"
...
// logServiceStartOfDay logs information about ltfnd when the main server has
...
// ltfndService houses the main service handler which handles all service
// updates and launching ltfndMain.
type ltfndService struct{}
...
// long-running ltfndMain (which is the real meat of ltfnd), handles service
// change requests, and notifies the service control manager of changes.
func (s *ltfndService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start ltfndMain in a separate goroutine so the service can start
...
// installService attempts to install the ltfnd service.  Typically this should
...
// removeService attempts to uninstall the ltfnd service.  Typically this should
...
// startService attempts to start the ltfnd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &ltfndService{})
	if err != nil {
