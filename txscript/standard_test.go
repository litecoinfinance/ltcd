...
// newAddressPubKey returns a new ltfnutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ltfnutil.Address {
	addr, err := ltfnutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new ltfnutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ltfnutil.Address {
	addr, err := ltfnutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new ltfnutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ltfnutil.Address {
	addr, err := ltfnutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the ltfnutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the ltfnutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ltfnutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := ltfnutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := ltfnutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := ltfnutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := ltfnutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*ltfnutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ltfnutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ltfnutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*ltfnutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*ltfnutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
