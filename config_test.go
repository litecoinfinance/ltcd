...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-ltfnd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "ltfnd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of ltfnd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-ltfnd.conf")
