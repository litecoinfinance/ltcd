...
import (
	...
	rpc "github.com/litecoinfinance/ltfnd/rpcclient"
	"github.com/litecoinfinance/ltfnutil"
)

// nodeConfig contains all the args, and data required to launch a ltfnd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	ltfndPath, err := btcdExecutablePath()
	if err != nil {
		ltfndPath = "ltfnd"
	}
...
// arguments returns an array of arguments that be used to launch the ltfnd
// process.
...
// command returns the exec.Cmd which will be used to start the ltfnd process.
...
// to the ltfnd process that is launched via Start().
...
// as the base for the log and data directories for ltfnd.
...
// start creates a new ltfnd process, and writes its pid in a file reserved for
...
// stop interrupts the running ltfnd process process, and waits until it exits
...
// shutdown terminates the running ltfnd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := ltfnutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
