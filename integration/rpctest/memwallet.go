...
type memWallet struct {
	...
	addrs map[uint32]ltfnutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]ltfnutil.Address)
...
// SetRPCClient saves the passed rpc connection to ltfnd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*ltfnutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          ltfnutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (ltfnutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []ltfnutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt ltfnutil.Amount,
	feeRate ltfnutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := ltfnutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt ltfnutil.Amount
	for _, output := range outputs {
		outputAmt += ltfnutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() ltfnutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance ltfnutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (ltfnutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := ltfnutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
