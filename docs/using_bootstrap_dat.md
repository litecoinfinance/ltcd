...
5. [How do I use bootstrap.dat with ltfnd?](#Importing)
...
### 5. How do I use bootstrap.dat with ltfnd?

ltfnd comes with a separate utility named `addblock` which can be used to import
...
1. Stop ltfnd if it is already running.  This is required since addblock needs to
   access the database used by ltfnd and it will be locked if ltfnd is using it.
