...
ltfnd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, ltfnd supports a hybrid,
...
Configuring ltfnd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the ltfnd command
line or in the ltfnd configuration file.  Typically the Tor proxy address will be
...
By default, ltfnd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since ltfnd will act as the server.
...
```bash
$ ./ltfnd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
ltfnd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the ltfnd command line or in the ltfnd
...
```bash
$ ./ltfnd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
ltfnd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./ltfnd --proxy=127.0.0.1:9050 --torisolation
```
