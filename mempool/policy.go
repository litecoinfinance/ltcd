...
const (
	...
	DefaultMinRelayTxFee = ltfnutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee ltfnutil.Amount) int64 {
	...
	if minFee < 0 || minFee > ltfnutil.MaxSatoshi {
		minFee = ltfnutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *ltfnutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee ltfnutil.Amount) bool {
...
func checkTransactionStandard(tx *ltfnutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee ltfnutil.Amount,
...
func GetTxVirtualSize(tx *ltfnutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
