...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: ltfnutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := ltfnutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*ltfnutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, ltfnutil.NewTx(tx))
	}
...
func additionalCoinbase(amount ltfnutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee ltfnutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee ltfnutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee ltfnutil.Amount) *wire.MsgTx {
