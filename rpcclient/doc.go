...
Bitcoin RPC server that uses a ltfnd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with ltfnd (https://github.com/litecoinfinance/ltfnd),
btcwallet (https://github.com/litecoinfinance/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, ltfnd and
...
ltfnd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by ltfnd and
...
All notifications provided by ltfnd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by ltfnd (and
...
Also, it is important to realize that ltfnd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the ltfnd RPC server directly, only the RPCs which are related to
...
The following full-blown client examples are in the examples directory:
 ...
 - ltfndwebsockets
   Connects to a ltfnd RPC server using TLS-secured websockets, registers for
