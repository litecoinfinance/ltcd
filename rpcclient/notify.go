...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *ltfnutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *ltfnutil.Tx, details *btcjson.BlockDetails)
	...
	// result of a rescan request, due to how ltfnd may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount ltfnutil.Amount)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case btcjson.BtcdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warnf("Received invalid ltfnd connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a ltfnd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*ltfnutil.Tx, error) {
	...
	transactions := make([]*ltfnutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = ltfnutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*ltfnutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return ltfnutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := ltfnutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseBtcdConnectedNtfnParams parses out the connection status of ltfnd
// and btcwallet from the parameters of a ltfndconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance ltfnutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := ltfnutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a ltfnd extension and requires a websocket connection.
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []ltfnutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []ltfnutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []ltfnutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a ltfnd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []ltfnutil.Address,
...
// NOTE: This is a ltfnd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []ltfnutil.Address, outPoints []wire.OutPoint) error {
