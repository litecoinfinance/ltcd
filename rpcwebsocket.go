...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *ltfnutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *ltfnutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *ltfnutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a ltfnutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := ltfnutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a ltfnutil.Address) bool {
	switch a := a.(type) {
	case *ltfnutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a ltfnutil.Address) {
	switch a := a.(type) {
	case *ltfnutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected ltfnutil.Block
type notificationBlockDisconnected ltfnutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *ltfnutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *ltfnutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *ltfnutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, ltfnutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*ltfnutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *ltfnutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *ltfnutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *ltfnutil.Tx, block *ltfnutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *ltfnutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *ltfnutil.Tx, block *ltfnutil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := ltfnutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *ltfnutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *ltfnutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *ltfnutil.Block) error {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	for _, addrStr := range cmd.Addresses {
		addr, err := ltfnutil.DecodeAddress(addrStr, params)
		if err != nil {
			jsonErr := btcjson.RPCError{
				Code: btcjson.ErrRPCInvalidAddressOrKey,
...
fetchRange:
	...
	// Notify websocket client of the finished rescan.  Due to how ltfnd
